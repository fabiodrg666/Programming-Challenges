/**
 * Determines whether a passphrase is valid, i.e. no two words are anagrams of each other
 * @param {Sring} passphrase the passphrase to be tested
 * @return {Boolean} true if the passphrase is valid, false otherwise
 */
var isValidPassphrase = function(passphrase) {
	if(typeof passphrase !== 'string')
		throw "Expected string";
	
	wordsArray = passphrase.split(' ');

	var foundAnagram = false;
	for(var i = 0; i < wordsArray.length && !foundAnagram; i++)
		for(var j = i + 1; j < wordsArray.length && !foundAnagram; j++)
			if(wordsArray[i].length === wordsArray[j].length) {
                // first test case, both words have the same length
                // next, let's see if both words contain the same characters
                var word1 = wordsArray[i], word2 = wordsArray[j];
                var equalChars = true;
                for(var k = 0; k < word1.length && equalChars; k++) {
                    // for each character in word1, try to find it on word2
                    // if all characters are found, word2 is an anagram of word1
                    var index;
                    if((index = word2.indexOf(word1.charAt(k))) === -1)
                        equalChars = false; // character not found, impossible to be anagram
                    else
                        word2 = word2.slice(0, index) + word2.slice(index+1); // removes already matched character
                }
...
/**
 * 
 * @param {String} allPassphrases 
 */
var countValidPassphrases = function(allPassphrases) {
	if(arguments.length === 0)
		throw "Missing mandatory argument";
	
	if(typeof allPassphrases !== 'string')
		throw "Expected string";

	var passphrasesArray = allPassphrases.split('\n');
	var counter = 0;
	passphrasesArray.forEach(passphrase => {
		// boolean 'true' is converted to 1 as a number
		// boolean 'false' is converted to 0 as a number
		counter += Number(isValidPassphrase(passphrase));
	});
...
/**
 * isValidPassphrase tests
 */
(function() {
	if(!isValidPassphrase("abcde fghij"))
		throw "Failed test 1";

	if(isValidPassphrase("abcde xyz ecdab"))
		throw "Failed test 2";
	
	if(!isValidPassphrase("a ab abc abd abf abj"))
        throw "Failed test 3";
        
    if(!isValidPassphrase("iiii oiii ooii oooi oooo"))
        throw "Failed test 4";
        
    if(isValidPassphrase("oiii ioii iioi iiio"))
		throw "Failed test 5";
})();
